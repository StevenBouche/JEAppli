...
class UserAdd extends Component {
    ...
    componentDidUpdate(){
        ...
        if( groupId !== this.state.groupId) this.setState({groupId: groupId},async () => {
    ...
    async handleTerm(term){
        if(this.state.term != term){
            this.setState({term: term},
                async () => {
                    var res = await TeamsManager.getUserGroupTeamComp(this.state.teamId, this.state.groupId, this.state.term)
    ...
    async handleAction(){
        if(this.state.idSelect != undefined){
            await TeamsManager.addUserGroupTeam(this.state.teamId, this.state.groupId, this.state.idSelect);
        }
        this.handleClose();
    }
    ...
    onRowSelectUser(row, isSelected, e) {
        if(this.state.idSelect != row.id) this.setState({idSelect: row.id});
      //  console.log(row._id)
    }
    ...
    render(){
        ...
        return (
            ...
            <ModalHeader toggle={this.state.callback}>Group modal</ModalHeader>
            <ModalBody>
            <Row>
                <Col xs="12" >
                <Card>
